...
export class ListComponent {
  ...
  calculateValues(): void {
    if(this.filteredMarkets.length>0){
      this.positiveCount = this.filteredMarkets.filter(market => (market.change24hPercent) > 0).length;
      this.positiveMarket = this.filteredMarkets.reduce((prev, current) => (+prev.change24hPercent > +current.change24hPercent) ? prev : current)
      this.negativeMarket= this.filteredMarkets.reduce((prev, current) => (+prev.change24hPercent < +current.change24hPercent) ? prev : current)
      this.count10000 = this.filteredMarkets.filter(market => (market.currentQuote) > 10000).length;
      this.count1 = this.filteredMarkets.filter(market => (market.currentQuote) > 1).length;
      let currentQuote = 0;
